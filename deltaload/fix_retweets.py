#!/usr/bin/env python3
"""
Fix retweet content in bookmarks by extracting original tweet text.

This script:
1. Identifies retweets in bookmark data
2. For each retweet:
   a. Tries to find the original tweet content in the user's cache files
   b. Falls back to direct extraction if not found in cache
3. Updates the bookmark with the original content
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("fix_retweets.log")
    ]
)
            ...
            if original_data and original_data['text']:
                ...
                if 'full_text' in bookmark:
                    ...
                    if len(parts) > 1:
                        ...
                        # If there's no structured content, just replace RT prefix
                        bookmark['full_text'] = original_data['text']
                ...
                if 'markdown' in bookmark:
                    ...
                    if len(parts) > 1:
                        ...
                        # If there's no structured content, just replace RT prefix
                        bookmark['markdown'] = original_data['text']
                ...
                if match:
                    ...
                    if 'full_text' in bookmark:
                        ...
                        if len(parts) > 1:
                            ...
                            # If there's no structured content, just replace RT prefix
                            bookmark['full_text'] = original_content
                    ...
                    if 'markdown' in bookmark:
                        ...
                        if len(parts) > 1:
                            ...
                            # If there's no structured content, just replace RT prefix
                            bookmark['markdown'] = original_content
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Fix retweet content in bookmarks")
    parser.add_argument('--input', '-i', default="data-bookmark.jsonl",
                      help="Input JSONL file containing bookmarks")
    parser.add_argument('--output', '-o', default="data-bookmark-fixed.jsonl",
                      help="Output JSONL file to write fixed bookmarks")
    ...
    if not cache_dir.exists():
        logger.warning(f"Cache directory does not exist: {args.cache_dir}")
